import type { SampleActivityItem } from "@/lib/sample-data";

export function ActivityFeedItem({ item }: { item: SampleActivityItem }) {
  return (
    <li className="flex items-start gap-3 border-b border-glass-border py-3 last:border-b-0">
      ...
      <span className="shrink-0 text-xs text-foreground/40">{item.timestamp}</span>
    </li>
  );
}
