  ...
  return <div className={cn("marco-shell", rail && "is-mini", !work && "no-work", drawer && "has-drawer", sheet && "has-sheet")}>
    ...
    <main className="marco-main">
      ...
      <section className={cn("marco-stage", view === "canvas" && !isLibrary && "is-canvas")}>
        {isLibrary ? <div className="marco-page-pad">{children}</div> : view === "chat" ? <ChatView messages={messages} agentColor={agent?.avatarColor} /> : view === "build" ? <BuildView run={run} /> : <CanvasView run={run} />}
      </section>
...
function BuildView({ run }: { run: Run | null }) { const manifestCount = run?.assetManifest.length ?? 0; const buckets = ["character", "style", "location", "product", "audio", "document"]; return <div className="marco-build"><BuildBlock n="1" title="Prompt"><p>{run?.title ?? "Describe the work in Chat, then shape its Run here."}</p><div className="marco-rolebar"><span>Rewrite with Voice</span><span>Load from Run</span></div></BuildBlock><BuildBlock n="2" title={`Asset manifest. ${manifestCount} items`}><div className="marco-manifest-buckets">{buckets.map((label, index) => <i key={label}><b>{manifestCount ? Math.max(1, Math.ceil(manifestCount / (index + 2))) : 0}</b><small>{label}</small></i>)}<i className="is-add">+ add</i></div><div className="marco-rolebar"><span>Bundle <b>current manifest</b></span><span>Reorder</span><span>Retag roles</span><span>Save as new bundle</span></div></BuildBlock><BuildBlock n="3" title="Model and output"><div className="marco-model-grid"><label>Model <b>Not configured</b><small>Connect a provider in Settings</small></label><label>Duration <b>Unavailable</b><small>Set by the provider model</small></label><label>Variations <b>{run?.outputs.length ?? 0}</b><small>Recorded outputs</small></label></div><div className="marco-aspect"><span>Aspect ratio</span><div><b>1:1</b><b>4:5</b><b className="is-selected">9:16</b><b>16:9</b><b>21:9</b></div></div></BuildBlock><div className="marco-runbar"><span>Provider cost <b>{run?.cost == null ? "Unknown" : `$${run.cost.toFixed(2)}`}</b><small>Generation is unavailable in Phase 4.6.</small></span><button disabled>Run</button></div></div>; }
