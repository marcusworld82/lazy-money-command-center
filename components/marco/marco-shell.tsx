...
import {
  BookOpen, BrainCircuit, CalendarDays, ChevronRight, Folder, Image as ImageIcon,
...
export function MarcoShell({ children }: { children: React.ReactNode }) {
  ...
  React.useEffect(() => {
    const stored = localStorage.getItem("marco:rail");
    if (stored === "off") setRailOpen(false);
  }, []);
  ...
  const filteredThreads = threads.filter((thread) => {
    ...
    const hay = `${thread.title ?? ""} ${row?.name ?? ""} ${thread.lastMessagePreview ?? ""}`.toLowerCase();
  ...
  function createGroup(event: React.FormEvent) {
    ...
    const thread: Thread = {
      ...
      sharedMemory: false,
      lastMessagePreview: "Private memories stay private",
      unread: false,
      ...
      <div className={cn("zy-window", railOpen && "has-rail", mode === "chat" && mobileThread && "is-thread")}>
        <header className="zy-top">
          <div className="zy-top-left">
            <button type="button" className="zy-icon-btn" onClick={toggleRail} aria-label={railOpen ? "Hide sidebar" : "Show sidebar"}>
              {railOpen ? <PanelLeftClose size={16} /> : <PanelLeftOpen size={16} />}
            </button>
            <Link href="/" className="zy-logo" onClick={() => switchMode("chat")}>
          ...
          <aside className="zy-rail">
            <div className="zy-rail-block">
              ...
              {agents.map((item) => {
                ...
                return (
                  <button key={item.id} type="button" className={cn("zy-rail-agent", agent?.id === item.id && mode === "chat" && "is-active")} onClick={() => thread && chooseThread(thread)}>
          ...
          {mode === "chat" ? (
            <>
              <section className="im-list">
                <header className="im-list-head">
                  <button type="button">Edit</button>
                  <h2>Messages</h2>
                  <button type="button" onClick={() => setGroupOpen(true)} aria-label="New message"><SquarePen size={18} /></button>
                </header>
                ...
                <div className="im-pins">
                  {agents.slice(0, 6).map((item) => {
                    const thread = threads.find((row) => row.agentId === item.id && row.kind !== "group") ?? threads.find((row) => row.agentId === item.id);
                    return (
                      <button key={item.id} type="button" onClick={() => thread && chooseThread(thread)}>
                        <AgentAvatar color={item.avatarColor} name={item.name} />
                        <span>{item.name}</span>
                      </button>
                    );
                  })}
              ...
              <section className="im-thread">
                <header className="im-thread-head">
                  ...
                  <div>
                    ...
                    <p>{activeThread?.kind === "group" ? participants.map((item) => item.name).join(" · ") : agent?.tagline ?? "Start a thread"}</p>
                  </div>
                </header>
            ...
            <main className="zy-build-main">
              {pathname === "/images" ? <StudioSurface kind="image" /> : pathname === "/video" ? <StudioSurface kind="video" /> : buildHome ? (
                <BuildHome onChat={() => switchMode("chat")} onImage={() => router.push("/images")} onVideo={() => router.push("/video")} />
              ) : (
        ...
        <div className="zy-modal-scrim" onClick={() => setGroupOpen(false)}>
          <form className="zy-modal" onClick={(event) => event.stopPropagation()} onSubmit={createGroup}>
            <header>
              ...
              <p>Agents share the brief in the room — not private Obsidian memory or skills.</p>
            </header>
              ...
              {agents.map((item) => {
                ...
                return (
                  <button key={item.id} type="button" className={on ? "is-on" : ""} onClick={() => setGroupPicks((current) => on ? current.filter((id) => id !== item.id) : [...current, item.id])}>
                    <AgentAvatar color={item.avatarColor} name={item.name} size="sm" />{item.name}
                  </button>
                );
              })}
            </div>
