...
async function apiKey() {
  const key = await resolveProviderKey("fal");
  if (!key) throw new Error("fal is not configured. Save a fal API key in Settings.");
  return key;
}

export async function listFalModels() {
  if (catalogCache && catalogCache.expiresAt > Date.now()) return catalogCache.models;
  const response = await fetch("https://fal.ai/api/models", { headers: { Authorization: `Key ${await apiKey()}` }, next: { revalidate: 3600 } });
...
export async function queueGeneration(request: GenerationRequest): Promise<GenerationResult> {
  ...
  const response = await fetch(`${FAL_QUEUE_URL}/${request.modelId}`, { method: "POST", headers: { Authorization: `Key ${await apiKey()}`, "Content-Type": "application/json" }, body: JSON.stringify(adapter.mapInput({ prompt, aspectRatio: request.aspectRatio, duration: request.duration, references: resolved.references })) });
...
export async function pollGeneration(modelId: string, requestId: string): Promise<GenerationResult> {
  const response = await fetch(`${FAL_QUEUE_URL}/${modelId}/requests/${requestId}/status`, { headers: { Authorization: `Key ${await apiKey()}` } });
  ...
  const resultResponse = await fetch(`${FAL_QUEUE_URL}/${modelId}/requests/${requestId}`, { headers: { Authorization: `Key ${await apiKey()}` } });
